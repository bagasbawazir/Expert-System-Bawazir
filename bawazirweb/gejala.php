<?php 
    ...
    include 'dbConfig/deleteGejala.php';
?>
<!doctype html>
<html lang="en">
<head>
                        ...
                        <div class="row align-items-center">
                            <div class="col-sm-12">
                                <div class="breadcrumbs-area clearfix">
                                    <h4 class="page-title pull-left">Daftar Gejala</h4>
                        ...
                        <div class="card">
                            ...
                            if(isset($_GET['message']) && !empty($_SESSION['i'])){
                                ...
                                <div class="alert alert-danger" role="alert">
                                    <?=$type=='a'?'Tambah':'Delete';$_SESSION['i'] = 0 ?> Gejala Gagal
                                </div>
                            <?php } else { ?>
                                <div class="alert alert-success" role="alert">
                                    <?=$type=='a'?'Tambah':'Delete';$_SESSION['i'] = 0 ?> Gejala Berhasil
                                </div>
                            <?php } 
                            } ?>
							<!-- <table id="example" class="table table-striped table-bordered nowrap" style="width:100%"> -->
                               <h4 class="header-title">
                                    Table Gejala
                                    <button type="button" class="btn btn-info btn-xs pull-right" data-toggle="modal" data-target="#modalTambah"> <i class="ti-plus"></i> Tambah Gejala</button>
                                </h4>
                                <div class="data-tables">
						          ...
						          <?php 
                                    ...
                                    $index = 1;
                                    $data = mysqli_query($conn,"select * from gejala order by kode");
                                    while($result = mysqli_fetch_array($data)){
                                    ?>
                                     <tr>
                                        <td><?php echo $index++; ?></td>
                                        <td><?php echo $result['kode'] ?></td>
                                        <td><?php echo $result['gejala'] ?></td>
                                        <td><a href='gejala.php?kode=<?php echo $result['kode'] ?>'>Delete</a> | <a href="gejala.php?edit=<?=$result['kode']?>">Edit</a></td>
                                    </tr>
                                    <?php 
                                        }
                                    ?>						
...
<div class="modal fade" tabindex="-1" role="dialog" id="modalTambah" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="post" action="dbConfig/addGejala.php">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="kode-gejala"> <strong> Kode Gejala </strong></label>
                        <input type="text" class="form-control" id="kode-gejala" name="kode" placeholder="G001">
                    </div>
                    <div class="form-group">
                        <label for="nama-gejala"> <strong> Nama Gejala </strong></label>
                        <input type="text" class="form-control" id="nama-gejala" name="nama" placeholder="Nama Gejala">
                    </div>
                </div>
    ...
    <script>
        <?php if(isset($_GET['edit'])){?>
            <?php $kode = $_GET['edit']; ?>
            <?php $data = mysqli_query($conn,"select * from gejala where kode = '$kode'"); ?>
            <?php $data  = mysqli_fetch_assoc($data)?>
            $('#kode-gejala').val("<?php echo $data['kode'] ?>");
            $('#nama-gejala').val("<?php echo $data['gejala'];?>");
            $('#act').val("update");
